...
## To Do
#- [ ] Allow users to create solutions (simple and mix ?)
#- [ ] Calculate the emissions after the solutions attribution
#- [ ] Display the impact of each solution
#- [ ] Display projected values by name and year
#- [ ] Display charts
#- [ ] Allow export of a file to avoid starting from scratch
...
from modules.tree import build_tree, create_projection_base
...
# Main tabs
tabs = st.tabs(["Home", "Growth", "Structural Effects", "Solutions", "Target Dashboard"])
...
with tabs[0]:
    st.title("Home: Upload an Excel file")

    # ============
    # Load saved session (JSON)
    # ============
    st.markdown("### Optional: Load a previously saved session")
    ...
    # Load new Excel file
    # ============
    st.write("Welcome! Please upload your Excel files using the required templates to get started.")
    ...
    if uploaded_file:
        try:
            ...
            if missing_cols:
                ...
                # Let user define years (only after Excel upload)
                start_year = st.sidebar.number_input("Start year", value=2025, step=1)
                end_year = st.sidebar.number_input("End year", value=2035, step=1, min_value=start_year)
...
# =========================================
# Tab 5: Target Dashboard – Save button
# =========================================
...
with tabs[4]:
    ...
    if has_loaded_data():
        ...
        session_to_export = {
            ...
            for k in keys_to_save if k in st.session_state
        }
