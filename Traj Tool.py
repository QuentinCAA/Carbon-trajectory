...
from modules.solutions import (
    ...
    compute_solution_impact_from_diagnostic, build_weights_summary_table , build_weights_debug_table, build_compute_debug_table, build_row_year_solution_debug
)
...
with tabs[4]:
    ...
    if has_loaded_data():
        ...
        if (
            impact_df.empty
            or df_emissions_before.empty
            or df_emissions_after.empty
            or df_avoided.empty
        ):
            st.info(
...
#                if isinstance(cell, list)
            ...
            # -------------------------------
            with st.expander(
                "🔎 Consistency check – mass balance of avoided emissions", expanded=False
            ):
                import numpy as np

                # a) Total avoided emissions per year from df_avoided
                emission_cols = [
                    f"Emissions_{y}" for y in years if f"Emissions_{y}" in df_avoided.columns
                ]

                if not emission_cols:
                    st.error(
                        "No emissions columns found in df_avoided – cannot run mass-balance check."
                    )
                else:
                    avoided_per_year_raw = df_avoided[emission_cols].sum(axis=0)
                    avoided_per_year = pd.Series(
                        {
                            y: avoided_per_year_raw.get(f"Emissions_{y}", 0.0)
                            for y in years
                        },
                        index=years,
                    ...
                    if not year_col_map:
                        st.error(
                            "Could not find any year-like columns in the solution impact table – "
                            "mass-balance check skipped."
                        )
                    else:
                        common_years = sorted(year_col_map.keys())
                        solutions_per_year_raw = impact_df[
                            [year_col_map[y] for y in common_years]
                        ].sum(axis=0)
                        solutions_per_year = pd.Series(
                            {
                                y: solutions_per_year_raw[year_col_map[y]]
                        ...
                        check_df = pd.concat(
                            [avoided_aligned, solutions_per_year], axis=1
                        )
                        check_df["Residual"] = (
                            check_df["Attributed to solutions"]
                            - check_df["Avoided emissions (before - after)"]
                        )
                        ...
                        with col_a:
                            st.metric(
                                "Total avoided emissions (before - after)",
                                f"{total_avoided:,.4f}",
                            )
                        with col_b:
                            st.metric(
                                "Total attributed to solutions",
                                f"{total_attributed:,.4f}",
                            )
                        with col_c:
                            st.metric(
                                "Residual (attributed - avoided)",
                                f"{total_residual:,.4f}",
                            )
                        ...
                        if abs(total_residual) <= atol:
                            ...
                            st.caption(
                                "Tip: check the debug weights and row-level debug above, especially for mixed "
                                "solutions, to see which rows/years receive unexpected weights."
                            )
