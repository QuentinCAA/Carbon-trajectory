...
def plot_cumulative_emissions_reduction(
    ...
    if show_percentage_annotation:
        ...
        ax.text(final_year, with_action + 0.05 * without,
                f"{percent_reduction:.0f}% reduction\nin {final_year}",
                color='red', fontsize=12, ha='right', va='bottom')

    # Aesthetics
    ax.set_title("Cumulative CO2e emissions with and without actions", fontsize=14)
    ax.set_xlabel("Year")
